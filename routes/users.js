...
.get(async (req, res) => {
    try {
        let user = await userData.getUserByEmail(req.session.user.email);
        let showGenderBool;
        let showPronounsBool;
        if (user.showGender == "on") showGenderBool = "checked";
        else showGenderBool = "unchecked";

        if (user.showPronouns == "on") showPronounsBool = "checked";
        else showPronounsBool = "unchecked";

        let isMan;
        let isWoman;
        let isOther;
        if (user.gender == "man") isMan = "checked";
        else isMan = "unchecked";

        if (user.gender == "woman") isWoman = "checked";
        else isWoman = "unchecked";

        if (user.gender == "other") isOther = "checked";
        else isOther = "unchecked";

        let isHe;
        let isShe;
        let isOtherPronoun;
        if (user.pronouns == "he/him") isHe = "checked";
        else isHe = "unchecked";

        if (user.pronouns == "she/her") isShe = "checked";
        else isShe = "unchecked";

        if (user.pronouns == "other") isOtherPronoun = "checked";
        else isOtherPronoun = "unchecked";

        res.render('users/updateSettings', {title : "Settings", firstName: user.firstName, birthday: user.birthday,
                                            aboutMe: user.about, showGender: user.showGender, showPronouns:user.showPronouns,
                                            showPronounsBool: showPronounsBool, showGenderBool: showGenderBool,
                                            isMan: isMan, isWoman: isWoman, isOther: isOther, isHe: isHe, isShe: isShe, 
                                            isOtherPronoun: isOtherPronoun});
    }
    catch(e){
        res.status(500).render('errors/error', {title : "Error", error : e.toString()});
    }
